...
public class SwipeMenuActivity extends BaseActivity implements View.OnClickListener{
    @ViewInject(R.id.main_swipemenu)
    SwipeMenu mMainSwipemenu;
    @ViewInject(R.id.menu_rg_trans)
    RadioGroup mRgTrans;
    @ViewInject(R.id.menu_cb_scale)
    CheckBox mCbScale;
    @ViewInject(R.id.menu_sb_scale)
    SeekBar mSbScale;
    @ViewInject(R.id.menu_cb_alpha)
    CheckBox mCbAlpha;
    @ViewInject(R.id.menu_sb_alpha)
    SeekBar mSbAlpha;
    @ViewInject(R.id.menu_rg_rotate)
    RadioGroup mRgRotate;
    @ViewInject(R.id.menu_Sb_rotate)
    SeekBar mSbRotate;
    @ViewInject(R.id.main_btn_menu)
    ImageButton mBtnMenu;
    @ViewInject(R.id.content_recyclerView)
    RecyclerView mContentRecyclerView;
    @ViewInject(R.id.cb_pic)
    CheckBox mCbPic;
    @ViewInject(R.id.content_rg_blur)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isPic == 0) {
            ...
            if (isBlur) {
                ...
                mMainSwipemenu.setBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary, 22f);
            } else if (isChangeBlur) {
                mContentRgBlur.check(R.id.content_rb_changeblur);
                mMainSwipemenu.setChangedBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary);
            } else if (isReverseChangeBlur) {
                mContentRgBlur.check(R.id.content_rb_reversechangeblur);
                mMainSwipemenu.setReverseChangedBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary);
            } else {

                mContentRgBlur.check(-1);
                mMainSwipemenu.setBackImage(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary);
            }
        }
    ...
    class TipAdapter extends RecyclerView.Adapter {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            if (position == 0) {
                ...
                if (mCbPic.isChecked()) {
                    builder.append("setBackImage(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary)\n");
                } else {
                ...
                if (mContentRgBlur.getCheckedRadioButtonId() == R.id.content_rb_blur) { //静态模糊
                    builder.append("setBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary, 22f)\n");
                }
                if (mContentRgBlur.getCheckedRadioButtonId() == R.id.content_rb_changeblur) { //动态模糊
                    builder.append("setChangedBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary)\n");
                }
                if (mContentRgBlur.getCheckedRadioButtonId() == R.id.content_rb_changeblur) { //动态模糊
                    builder.append("setReverseChangedBlur(SwipeMenuActivity.this, R.mipmap.dayu, R.color.colorPrimary)");
                }
    ...
    class TipViewHolder extends RecyclerView.ViewHolder {
        @ViewInject(R.id.tip_tv_content)
        TextView mTvContext;
